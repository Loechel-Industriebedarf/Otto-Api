<?php
    ...
    if(file_exists($csvPathOrders)){
		...
		$lastDateTimeMod = $lastDateTime ->modify('-31 minutes');
		
		$lastDateTimeModStr = $lastDateTimeMod->format('Y-m-d\TH:i:s.000+0000');
		
		echo $lastDate . "<br><br>" . $lastDateTimeModStr . "<br><br>";

        //Check if new orders exist
        $orders = getOrders($url, $accessToken, $lastDateTimeModStr);
    ...
    function getOrders($url, $accessToken, $lastDate){
        ...
        $requestUrl = $url . '/v4/orders?fromOrderDate=' . urlencode($lastDate) . '&fulfillmentStatus=PROCESSABLE';
        echo $requestUrl;
